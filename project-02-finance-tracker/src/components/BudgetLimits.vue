      ...
      >
        <div class="flex items-center justify-between mb-3">
          <div class="flex items-center gap-2">
            ...
            <span class="text-sm font-semibold text-slate-900">{{ item.cat }}</span>
          </div>
          <button
            @click="startEdit(item.cat, item.usage.limit)"
        ...
        <div v-if="editingId === item.cat" class="mb-3">
        ...
        <div>
          <div class="flex justify-between text-xs mb-1.5">
            <span class="font-medium text-slate-700">
              {{ fmt(item.usage.spent) }}
              <span class="text-slate-400 font-normal"> / {{ fmt(item.usage.limit) }}</span>
            </span>
            <span class="font-semibold" :class="item.statusColor">{{ item.usage.pct }}%</span>
          </div>
          <div class="w-full bg-slate-100 rounded-full h-2 overflow-hidden">
            <div
              class="h-2 rounded-full transition-all duration-500"
              :style="{ width: `${item.usage.pct}%`, backgroundColor: item.barColor }"
            ></div>
          </div>
          <p v-if="item.usage.pct >= 100" class="text-xs text-red-600 font-medium mt-1.5 flex items-center gap-1">
            <svg class="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 20 20">
              <path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd" />
            </svg>
            Plafond dépassé !
          </p>
          <p v-else-if="item.usage.pct >= 75" class="text-xs text-amber-600 mt-1.5">
            Attention, bientôt atteint
          </p>
          <p v-else-if="item.usage.limit === 0" class="text-xs text-slate-400 mt-1.5">
            Aucun plafond défini
          </p>
          <p v-else class="text-xs text-slate-400 mt-1.5">
            {{ fmt(Math.max(0, item.usage.limit - item.usage.spent)) }} restants
          </p>
        </div>
      </div>
    </div>
...
const budgetItems = computed(() =>
  EXPENSE_CATEGORIES.map((cat) => {
    const usage = store.getBudgetUsage(cat)
    const statusColor =
      usage.pct >= 100 ? 'text-red-600' : usage.pct >= 75 ? 'text-amber-500' : 'text-emerald-600'
    const barColor =
      usage.pct >= 100 ? '#ef4444' : usage.pct >= 75 ? '#f59e0b' : CATEGORY_COLORS[cat]
    return { cat, usage, color: CATEGORY_COLORS[cat], statusColor, barColor }
  })
)

function startEdit(cat: Category, currentLimit: number) {
  editingId.value = cat
  editValue.value = currentLimit || 200
